...
auto mf::WlSurface::transform_point(
    geom::Point point) -> std::experimental::optional<std::pair<WlSurface*, geom::Point>>
{
    ...
    {
        if (auto result = (*child_it)->transform_point(point))
            return result;
    }
    geom::Rectangle surface_rect = {geom::Point{}, buffer_size_.value()};
    for (auto& rect : input_shape.value_or(std::vector<geom::Rectangle>{surface_rect}))
    {
        if (rect.intersection_with(surface_rect).contains(point))
            return std::make_pair(this, point);
    }
    return std::experimental::nullopt;
}
